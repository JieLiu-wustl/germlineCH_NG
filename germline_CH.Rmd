...
# cannot to shared 
M_wide = readRDS("Inputs/M_wide_050824.rds")
...
# cannot to shared
germline_c = readRDS("Inputs/germline_050824.rds")

# individual-level CH calls
# cannot to shared
ch_c = readRDS("Inputs/ch_050824.rds")
mCA = readRDS("Inputs/mCA_050824.rds") %>% mutate(eid=as.character(eid))
...
# blood counts
# individual-level, cannot be shared
bc=readRDS("Inputs/blood_z_050824.rds")
...
# pdf(paste0(dir,"Figures/","1a.pdf"), width = 100/25.4, height = 115/25.4)
# par(mar=c(0,0,0,0))
# draw(p, heatmap_legend_side ="top")
# dev.off()

png(paste0(dir,"Figures/","1a.png"), width = 100, height = 115, units = "mm", res = 300)
par(mar=c(0,0,0,0))
draw(p, heatmap_legend_side ="top")
dev.off()
...
plot = readRDS("Inputs/g_heme.rds") %>%
  ...
  filter(germline%in%germline[p.value<0.05]) %>%
  #filter(germline%in%germline[estimate>1]) %>%
...
p <- plot %>%
  ...
  geom_blank() +
  geom_hline(yintercept = 1, color = "gray", linetype = "solid") +
  geom_text(position = position_dodge(width = 0.6), size = 5/.pt, alpha = .9, angle=90, hjust=0.5,vjust=0,show.legend = FALSE) +
...
savePdf(p,w=120,h=80,o="Fig3.png")
...
cut_off = 0.01
(risk %>% filter(Population=="Non-carriers") %>% filter(risk>=cut_off) %>% nrow())/nrow(risk%>% filter(Population=="Non-carriers")) # 99% risk < 1%
(risk %>% filter(Population=="CHEK2 carriers") %>% filter(risk>=cut_off) %>% nrow())/nrow(risk%>% filter(Population=="CHEK2 carriers")) # 8%
(risk %>% filter(Population=="ATM carriers") %>% filter(risk>=cut_off) %>% nrow())/nrow(risk%>% filter(Population=="ATM carriers")) # 4%
...
table(plot$Population,plot$Percentile)
plot%>%filter(Percentile==100) %>% group_by(Population) %>% summarise(m=median(risk)) %>% as.matrix()

plot %>% filter(Percentile>=99.5) %>% group_by(Population,Percentile) %>% summarise(med=median(risk)) %>% as.matrix()
...
p4 <- ggplot(plot) + 
  geom_col(aes(x=num_bin, y=prop,fill=as.factor(germline)), color="black", linewidth=0.5/.pt,
   ...
   geom_text(inherit.aes = F,data=pval, mapping = aes(x = num_bin, y = ypos+0.3, label = p.stars),
            size = 5/.pt, fontface = "bold") +
  ...
  scale_fill_manual(name = "", labels = c("No germline variant", "Germline variant"), values = c("dark grey",colors[1]))
...
ggplot(
  ...
  annotate("text", x = 1.5, y = 0.8, label = pval$p.stars, vjust = -1, size=5/.pt) +
...
BER=("MUTYH","NTHL1","POLD1","POLE","POLH","WRN")
NER=("ERCC1","ERCC2","ERCC3","ERCC4","ERCC5","POLD1","POLE","XPA","XPC")
...
#savePdf(p=p,w=120,h=100,o="Efig5.pdf")
ggsave(paste0(dir,"Figures/png/","Efig5.png"), plot = p, width = 120, height = 80, units = c("mm"), dpi = 300, device = cairo_pdf)
...
E3 = ggarrange(panel_theme(p1,"A"),panel_theme(p2,"B"),nrow = 2,common.legend = T, legend = "top")
savePdf(p=E3,w=89,h=70,o="Efig3.png")
